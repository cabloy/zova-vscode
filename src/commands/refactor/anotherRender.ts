...
export async function refactorAnotherRender(resource?: Uri) {
  ...
  // invoke
  await invokeZovaCli(
    [':refactor:anotherRender', pathResource, name, `--module=${commandPathInfo.moduleName}`, '--nometadata'],
    commandPathInfo.projectCurrent,
  );
  // metadata
  invokeToolsMetadata(commandPathInfo.moduleName, commandPathInfo.projectCurrent);
  // open
  const fileDest = path.join(commandPathInfo.moduleRoot, `src/${pathResource}/render.${name}.tsx`);
