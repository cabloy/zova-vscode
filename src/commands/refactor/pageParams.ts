...
export async function refactorPageParams(resource?: Uri) {
  ...
  // pathResource
  let pathResource = trimPathPrefixs(commandPathInfo.pathResource, ['src/page/', 'src/']);
  pathResource = pathResource.split('/')[0];
  // invoke
  await invokeZovaCli([':refactor:pageParams', pathResource, `--module=${commandPathInfo.moduleName}`], commandPathInfo.projectCurrent);
  // metadata
  invokeToolsMetadata(commandPathInfo.moduleName, commandPathInfo.projectCurrent);
  // open
  let fileDest = path.join(commandPathInfo.projectCurrent, commandPathInfo.moduleRoot, `src/page/${pathResource}/controller.ts`);
